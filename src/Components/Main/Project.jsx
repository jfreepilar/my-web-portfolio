    ...
    <div className='px-28 pt-40 pb-10 mt-5 mb-20 text-gray-500' id='project'>
      ...
      <div className='flex flex-wrap justify-evenly gap-10'>
        ...
        {projects.map((item, index) => (
                  <div className='border-2 w-[450px] mx-auto mt-8  border-gray-300 flex flex-col gap-4' key={index}>
                    <img className={`w-[450px] h-[250px] border-b-[5px] ${item.borderColor}`} src={item.image} alt={item.alt} />
                    <div className='px-12 h-min-fit'>
                      <p className='text-center text-2xl'>{item.name}</p>
          
                      <p className='text-[15px] leading-relaxed mt-1'>{item.description}</p>
